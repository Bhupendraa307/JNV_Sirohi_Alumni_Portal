...
//             res.cookie('visitorId', Date.now(), { maxAge: 31536000000, httpOnly: true });
//         }
...
io.on('connection', (socket) => {
    ...
    // Send updated stats to everyone when a user connects
    ...
    socket.on('disconnect', () => {
        ...
        // Send updated stats to everyone when a user disconnects
        SiteStats.findOne().then(stats => {
            if (stats) {
                io.emit("stats_update", {
                    activeUsers: userSockets.size,
